            ...
            <div class="row">
                    ...
                    <div class="col-md-8 pl-0">
                        <div class="card">
                            ...
                            <?php endif; ?>
                            <div class="card-header"><?php echo h($other_user_data['user_id']); ?>さんへメッセージを送ります。</div>
                            <div class="card-body">
                                 <!-- メッセージ入力フォーム -->
                                <form class="row form-inline message" method="post">
                                    <input type="hidden" name="process_kind" value="message">    
                                    <input type="hidden" name="other_user_id" value="<?php echo $other_user_data['id']; ?>">
                                    <input class="form-control col-md-4 col-8" type="text" name="comment" placeholder="メッセージを入力してください">
                                    <button type="submit" class="btn btn-primary col-md-2 col-4">送信</button>
                                </form>
                                <?php if(!empty($message['comment'])): ?>
                                ...
                                <?php foreach($comments_data as $comment_data): ?>
                                    <div class="row content">
                                        <div class="col-3 text-center">
                                            <img style="width:50px;" src="./img/<?php echo h($comment_data['img']); ?>">
                                            <p class="text-center small"><?php echo h($comment_data['user_id']); ?></p>
                                        </div>
                                        <div class="col-9">
                                            <p class="comment"><?php echo h($comment_data['comment']); ?></p>
                                            <p class="createdatetime">投稿日時:<?php echo h($comment_data['createdatetime']); ?></p>
                                        </div>
                                    </div>
                                    <hr>
                                <?php endforeach; ?>
